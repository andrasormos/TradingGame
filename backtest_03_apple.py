...
if 1==2:
	...
	df_filled = pd.DataFrame(columns=["Unix", "Date", "Close", "Volume"])
	#len(df_BTC)
	# unix_time = unix_time + 900
	# df_filled["Unix"][i] = unix_time
...
df_BTC_1 = pd.read_csv("./cryptoExtract/AAPL/AAPL_15min_filled.csv", index_col=0)
df_BTC_2 = pd.read_csv("./cryptoExtract/AAPL/AAPL_15min.csv", index_col=0)

# df_BTC_1 = df_BTC_1.iloc[::-1].reset_index(drop=True)
# df_BTC_2 = df_BTC_2.iloc[::-1].reset_index(drop=True)

df_BTC_1 = df_BTC_1.loc[:1703]
df_BTC_2 = df_BTC_2.loc[26:]
