...
# df_BTC["Sma"] = df_BTC["Close"].rolling(window=ma_a, min_periods=0).mean()
# df_BTC["Ema"] = df_BTC["Close"].rolling(window=ma_b, min_periods=0).mean()
								 #.ewm(span=200,adjust=False, min_periods=200-1).mean()
...
df_BTC["Sma"] = df_BTC["Close"].ewm(span=ma_a, min_periods=200-1, adjust=False, ignore_na=False).mean()
df_BTC["Ema"] = df_BTC["Close"].ewm(span=ma_b, min_periods=200-1, adjust=False, ignore_na=False).mean()
