...
class PlayGame(object):
    def __init__(self):
        ...
        self.trainingLogger = True
        self.logName = "trainLog_001.csv"
        self.trainingLog = pd.DataFrame(columns=["rewardSum", "profit", "guessedRightCnt", "guessedWrongCnt", "skipCnt", "guessCnt"])
        
        self.guessedRightCnt = 0
        self.guessedWrongCnt = 0
        self.skipCnt = 0
        self.guessCnt = 0
        self.guessLog = pd.DataFrame(columns=["guessedRightCnt", "guessedWrongCnt", "skipCnt", "guessCnt"])
        self.rwslogCnt = 0
    ...
    def nextStep(self, action):
        ...
        if self.trainingLogger == True:
            self.rewardSum = self.rewardSum + self.reward
                if self.done == True:

                # REWARD AND PROFIT LOG
                self.rewardScoreLog.loc[self.rwslogCnt] = self.rewardSum, self.profit, self.guessedRightCnt, self.guessedWrongCnt, self.skipCnt, self.guessCnt
                self.rwslogCnt += 1
                self.rewardScoreLog.to_csv(self.logName, index=True)
                self.rewardSum = 0
                self.guessedRightCnt = 0
                self.guessedWrongCnt = 0
                self.skipCnt = 0
